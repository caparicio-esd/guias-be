...
class GuiaChronogramTest(TestCase):
    ...
    def test_get_endpoints(self):
        #
        response = self.client.get("/api/v1/chronograms/1/")
        data = response.json()
        ...
        #
        response = self.client.get("/api/v1/chronograms/1/chronogramblocks/")
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(type(data), list)
        self.assertEqual(len(data), 18)
        #
        response = self.client.get("/api/v1/chronograms/1/chronogramblocks/1/")
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(type(data), dict)
    ...
    def test_post_endpoints(self):
        response = self.client.post("/api/v1/chronograms/")
        data = response.json()
        ...
        response = self.client.post("/api/v1/chronograms/2/chronogramblocks/", data=response_data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_put_endpoints(self):
        ...
        response = self.client.put("/api/v1/chronograms/1/chronogramblocks/1/", data=response_data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_patch_endpoints(self):
        ...
        response = self.client.patch("/api/v1/chronograms/1/chronogramblocks/1/", data=response_data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data["id"], 1)
        self.assertEqual(data["theme"], 2)
    ...
    def test_delete_endpoints(self):
        #
        response = self.client.delete("/api/v1/chronograms/1/")
